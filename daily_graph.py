...
def draw_graph():
    ...
    try:
        ...
        font = ImageFont.truetype(r"./UDDigiKyokashoN-R.ttc",size=16) #使用フォントを指定。Windows標準の位置にないとバグる
...
def twitter_connection():
    ...
    with open(f"./{today}.png", mode="rb") as image:
        image = image.read()
    ...
    twitter_token=""
    token_secret=""
    #当初os.getenv()で取っていましたがなぜかNoneを返しやがるので直書きに変更しました
    #書いてあると思った人、残念だったなwww
...
def push_files():
    ...
    try:
        today = datetime.date.today().strftime(r"%Y%m%d") #str型の今日の日付
        source = "./player_data.json"
        copy = f"./player_data{today}.json"
        shutil.copy(source, copy)
        with open(f"player_data{today}.json", mode="r") as f0: #本日の
        ...
        os.remove(f"player_data{today}.json") #ファイルが増え続けていくので送信したら消す
        os.remove(f"{today}.json") #同上
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                soup = bs4.BeautifulSoup(res.text, "html.parser") #特に意味のないコメント(けい制作srcコピペ防止)
                player_data_every_hour_dict = json.loads(soup.decode("utf-8")) #全体を辞書型で取得
            ...
            #ウェブページに不調があったときの処理
                ...
                now_hour = datetime.datetime.now().hour #今何時？
                one_hour_ago = now_hour - 1 #1時間前は何時？
                for player_data in player_data_dict.values(): #全データを一つずつ取り出す
                    player_data[f"{now_hour}"] = player_data[f"{one_hour_ago}"] #1時間前と同じ値を記録
                break
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                for player_data_every_hour in player_data_every_hour_list: #リストから一つずつデータを抜き出す
                    ...
                    try:
                        ...
                        player_data = {}
                    player_data["23_58"] = int(raw_data) #プレイヤーの時間毎の整地量が格納される辞書
                    player_data_dict[mcid] = player_data #上記の辞書を全MCIDぶん格納する辞書
            ...
            #ウェブページに不調があったときの処理
                ...
                for player_data in player_data_dict.values(): #全データを一つずつ取り出す
                    player_data["23_58"] = player_data["23"] #1時間前と同じ値を記録
                break
...
def job_every_day():
    """
    毎日の処理
    毎日23時58分に整地鯖ランキングページにアクセスし、
    job_23_58を実行したのち上位20人のMCID・整地量・順位を取得。"""
    ...
    try:
        ...
        try:
            ...
            soup = bs4.BeautifulSoup(res.text, "html.parser")
            player_data_every_day_dict = json.loads(soup.decode("utf-8")) #全体を辞書型で取得
